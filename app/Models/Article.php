...
class Article extends Model
{
    ...
    protected $fillable = [
        'title', 'status', 'url', 'description', 'sort_order', 'img_service', 'image', 'banner', 'meta_description', 'created_at', 'updated_at', 'deleted_at'
    ];
    ...
    public function getArticle($search = null, $skip, $perPage)
     {
         ...
         if (is_array($search) && count($search) > 0) {
             $keyword = (array_key_exists('keyword', $search)) ?
                 " AND (articles.title LIKE '%" .addslashes(trim($search['keyword'])) . "%')" : "";
             $filter .= $keyword;
         }
